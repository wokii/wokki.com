...
export default function ConsultancyHeader({
  ...
  const { header } = Zen[CONSULTANCY_WOKKI];
    ...
    <header className="fixed top-0 left-0 z-50 w-full border-b border-foreground/10 bg-background/70 backdrop-blur-xl">
      <div className="mx-auto flex max-w-5xl items-center justify-between px-6 py-3">
        <div className="group relative w-72">
          ...
          >
            <span className="whitespace-nowrap">{header.brand}</span>
          </a>
            ...
            <div className="flex flex-col gap-1.5">
              {header.links.map((link) => (
                <a
                  ...
                  className={
                    link.featured
                      ? "group/item block w-full rounded-2xl border border-accent/20 bg-accent/[0.06] px-3 py-2.5 transition-all duration-200 hover:border-accent/35 hover:bg-accent/[0.12]"
                      : "group/item block w-full rounded-2xl border border-transparent px-3 py-2.5 transition-all duration-200 hover:border-foreground/10 hover:bg-foreground/[0.04]"
                  }
                >
                  <p
                    className={
                      link.featured
                        ? "text-base font-medium text-foreground/90 transition-colors group-hover/item:text-accent"
                        : "text-base font-medium text-foreground/85 transition-colors group-hover/item:text-foreground"
                    }
                  >
                    {link.label}
                  </p>
                  <p
                    className={
                      link.featured
                        ? "text-[10px] uppercase tracking-[0.2em] text-accent/75"
                        : "text-[10px] uppercase tracking-[0.2em] text-foreground/45"
                    }
                  >
                    {link.tag}
                  </p>
